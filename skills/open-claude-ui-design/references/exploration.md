...
Exploration is complete when each option differs in one sentence, its tradeoff is visible, the recommendation is explicit, its review location is unambiguous, and the user has chosen a direction or explicitly delegated that choice. The design phase is complete only after the selected direction replaces the draft set and the full requested deliverable passes render and readback verification. Implementation remains blocked before both gates.
