...
Use the driver named by the repository. For browser pages, prefer the installed browser automation available to the active agent; for installed or mobile UI, use the repository's simulator or device workflow.
...
The screenshot is the visual ground truth. Keep the concrete acceptance points beside it while judging the result. DOM measurements and computed styles diagnose why clipping, overflow, or spacing is wrong; they do not overrule visible evidence. If three correction rounds fail to converge on the same defect, measure the affected element and parent, state the root cause, and make one structural correction rather than accumulating cosmetic offsets.

## Deterministic detector

After runtime interaction and visual checks pass, reuse current findings from Impeccable's installed edit/stop hook when available. Run `impeccable detect --json <explicit-ui-target>` only when no current hook evidence exists or when rechecking a specific fix. Treat findings as advisory evidence and judge them against the product's actual design system.
